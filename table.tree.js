    ...
    $(document).ready(function() {
        ...
        function createRows() {
            ...
            for (var i = 0; i < opts.length; i++) {
                ...
                // Do not hide the super parent elements (those without parentId)
                if (item.level > 0) {
                    $(trEle).addClass('hidden'); // Initially hide children rows
                }
                ...
                for (var j = 0; j <= item.level; j++) {
                    ...
                    if (j === item.level) {
                        if (item.children && item.children.length > 0) {
                            ...
                            $(spanEle).attr('data-is-open', '0'); // Set to collapsed
                            $(spanEle).attr('data-count', countChildren(item));
                            $(spanEle).text('+'); // Set text to '+' indicating collapsed
                        } else {
